...
Reticulator = function (options) {
  ...
  this.options = merge({
    width: 960,
    columns: 4, // => 0 if you want to create an empty base
    gutter: 20,
    offset: 0,
    baseline: 20,
    align: "center",
  ...
  */
  buildGrid : function() {
    ...
    this.basegrid = {
      ...
      height: (document.documentElement.clientHeight > document.body.scrollHeight ? document.documentElement.clientHeight : document.body.scrollHeight) // document height
    };
    ...
    if (this.options.baseline !== 0) {
      for (n = this.options.baseline; n < this.basegrid.height; n += this.options.baseline) {
        // horizontal guides here
        this.addHorizontalGuide(n-1);
      }
    }
